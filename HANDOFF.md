...
- 국문 자간 +0.96은 적용 완료 (`ls` 속성, ctx.letterSpacing — 원본 대비 폭 차 0.36pt).
  영문 자간(-0.27/-0.12)은 실폰트(Bold/Medium 웨이트) 확보 후 적용할 것 — 대체폰트에
  자간까지 얹으면 보정이 틀어짐.
- 현재 영문 대체: Helvetica Neue/Arial. 위 표의 baseline 좌표(코드 값)는 대체폰트 기준으로
  원본 렌더와 밴드 비교하여 보정된 값 (원본 raw 값과 다름 주의)
