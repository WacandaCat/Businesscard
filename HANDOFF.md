...
2. **접근 정책 결정**: 팀원 사용 시 Vercel 계정 공유 불가 → 옵션: (a) 사내 IP Trusted IPs,
   (b) 비밀번호 보호(유료), (c) 명단 데이터를 코드에서 분리(Supabase)하고 페이지는 공개
3. **명단 영속화**: 현재 roster는 코드에 하드코딩(9명, 2026-01-21 기준). 수정하려면
   index.html의 `roster` 배열 편집. 장기적으로 Supabase 연동 검토 (localStorage는
   회사 규칙상 팀 사용 시 기기별 분산 문제 있음)
4. **재단선 옵션 검증**: crop marks 켠 출력을 인쇄소에 1회 확인
